...
DEVICES := -m 256M -device ramfb -device virtio-tablet-device -device virtio-keyboard-device -netdev user,id=n0 -device virtio-net-device,netdev=n0 -device virtio-rng-device -global virtio-mmio.force-legacy=false -drive file=$(DISK),if=none,format=raw,id=hd0 -device virtio-blk-device,drive=hd0
